...
const MIGRATION_ID$4 = "001_initial";
const SCHEMA_SQL$4 = `
  -- ============================================================
...
const MIGRATION_ID$3 = "002_add_profile_facts";
const SCHEMA_SQL$3 = `
  -- ============================================================
...
const MIGRATION_ID$2 = "003_add_contact_profiles";
const SCHEMA_SQL$2 = `
  -- ============================================================
...
const MIGRATION_ID$1 = "004_parent_child_chunks";
const SCHEMA_SQL$1 = `
  -- ============================================================
...
class ChunkRepository {
   ...
   */
  insertParentChildBatch(parents, children) {
    ...
    const insertFts = this.db.prepare(`
      ...
      VALUES (@content, @chunk_id)
    `);
    const runAll = this.db.transaction((ps, cs) => {
  ...
  findParentsByChildIds(childIds) {
  ...
  findParentMapByChildIds(childIds) {
    if (childIds.length === 0) return /* @__PURE__ */ new Map();
    const placeholders = childIds.map(() => "?").join(", ");
    const rows = this.db.prepare(`
...
class VectorRepository {
   ...
   */
  search(queryEmbedding, topK = 10, chatId, dateFrom, dateTo) {
    if (!this.isAvailable) {
    ...
    `;
    const params = [buffer];
    if (chatId || dateFrom !== void 0 || dateTo !== void 0) {
   ...
   */
  hybridSearch(queryEmbedding, queryText, topK = 10, alpha = 0.7, chatId, dateFrom, dateTo) {
  ...
  hybridSearchPropositions(queryEmbedding, queryText, topK = 10, alpha = 0.7, chatId, dateFrom, dateTo) {
    return this._hybridSearchCore(
      "propositions",
      "proposition_id",
      "proposition_vectors",
  ...
  _hybridSearchCore(baseTable, idColumn, vecTable, ftsTable, queryEmbedding, queryText, topK, alpha, chatId, chatIdColumn = "chat_id", ftsContentColumn = "content", dateFrom, dateTo) {
    if (!this.isAvailable) {
...
const DEFAULT_CHUNKING_CONFIG = {
  ...
  // 10 minutos
  maxTokens: 1024,
  // (era 512, aumentado para ser o parent)
  overlapMessages: 5
  // Overlap do parent
};
const CHILD_CHUNKING_CONFIG = {
  windowMessages: 8,
  strideMessages: 5
  // Overlap de 3
};
class ParentChildStrategy {
  constructor(config) {
    __publicField(this, "baseStrategy");
  ...
  chunkWithMessages(messages, config) {
    if (messages.length === 0) return [];
    const chunksAndMessages = [];
    let windowMessages = [];
    const flushChunk = () => {
      if (windowMessages.length === 0) return;
  ...
  formatContent(messages) {
    return messages.filter((m) => m.type !== "system").map((m) => `${m.sender}: ${m.content}`).join("\n");
  }
  formatDisplayContent(messages) {
...
class PropositionExtractor {
  constructor() {
    ...
    __publicField(this, "PROMPT", `Leia o bloco de mensagens e extraia os fatos auto-contidos mais importantes (Proposições Atômicas).
...
class ChatImportService {
  ...
  async import(filePath, sender) {
    ...
    try {
      ...
      const newParents = chunkResult.parents.map((c) => ({
        id: c.id,
        chat_id: chatId,
        content: c.content,
      ...
      const newChildren = chunkResult.children.map((c) => ({
        id: nanoid(),
      ...
      if (!isAvailable) {
        ...
        await modelManager.download("embedding", (progress) => {
          ...
          emit({
            stage: "embedding",
            percent: 75 + Math.round(progress.percent * 0.05),
            // from 75 to 80%
...
const _SearchService = class _SearchService {
  ...
  async search(query, options, precomputedEmbedding) {
    ...
    const dateFrom = options == null ? void 0 : options.dateFrom;
    const dateTo = options == null ? void 0 : options.dateTo;
    ...
    let propParentMap = /* @__PURE__ */ new Map();
    if (propIds.length > 0) {
      const db = DatabaseService.getInstance();
      const ph = propIds.map(() => "?").join(", ");
      const props = db.prepare(`SELECT * FROM propositions WHERE id IN (${ph})`).all(...propIds);
    ...
    const allFacts = factIds.length > 0 ? this.factRepo.findByChatId(chatId || (sampleParent == null ? void 0 : sampleParent.chat_id) || "") : [];
    ...
    for (const res of sortedMerged) {
      if (res.type === "chunk") {
        ...
        if (prop.fact_date) dateStr = prop.fact_date;
          ...
          const date = new Date(parentChunk.start_time * 1e3);
...
Contexto Original:
${(parentChunk == null ? void 0 : parentChunk.display_content) || prop.original_quote}`;
        finalResults.push({
          id: `prop-${prop.id}`,
          chatId: prop.chat_id,
          chatName,
          ...
          chunkId: (parentChunk == null ? void 0 : parentChunk.id) || prop.id
        });
      } else {
...
const _TemporalResolver = class _TemporalResolver {
  constructor() {
    __publicField(this, "llm");
    __publicField(this, "RESOLVE_PROMPT", `Hoje é {today}.
...
`);
  ...
  static getInstance() {
    if (!_TemporalResolver.instance) {
      _TemporalResolver.instance = new _TemporalResolver();
    }
    return _TemporalResolver.instance;
  }
  async resolve(question) {
    const today = new Intl.DateTimeFormat("en-CA", { year: "numeric", month: "2-digit", day: "2-digit" }).format(/* @__PURE__ */ new Date());
...
__publicField(_TemporalResolver, "instance", null);
let TemporalResolver = _TemporalResolver;
const _MultiQueryRetriever = class _MultiQueryRetriever {
  constructor() {
    __publicField(this, "llm");
    __publicField(this, "search");
    __publicField(this, "embeddingService");
    __publicField(this, "REWRITE_PROMPT", `Você vai ajudar a buscar informações em conversas de WhatsApp. 
...
`);
  ...
  static getInstance() {
    if (!_MultiQueryRetriever.instance) {
      _MultiQueryRetriever.instance = new _MultiQueryRetriever();
    }
    return _MultiQueryRetriever.instance;
  }
  async retrieve(question, topK, contactName = "Desconhecido", chatId, dateFrom, dateTo) {
    ...
    for (const query of queries) {
      ...
      let queryEmbedding = new Float32Array(384);
      try {
        queryEmbedding = await this.embeddingService.embed(queryTrimmed);
      } catch (err) {
    ...
    const rankedResults = Array.from(allResults.values()).sort((a, b) => {
      if (Math.abs(b.score - a.score) < 1e-3) {
    ...
    for (const res of finalResults) {
      res.score = Math.min(1, res.score / maxScore);
    }
    return finalResults;
  }
};
__publicField(_MultiQueryRetriever, "instance", null);
let MultiQueryRetriever = _MultiQueryRetriever;
  ...
  async generateStream(question, onToken, options) {
    ...
    try {
      ...
      if (temporal.has_temporal_filter && temporal.date_from && temporal.date_to) {
        dateFrom = Math.floor(new Date(temporal.date_from).getTime() / 1e3);
        dateTo = Math.floor(new Date(temporal.date_to).getTime() / 1e3);
...
${contactProfile.profile_text}`,
        ...
        context = await multiQuery.retrieve(
          cleanQuestion,
          config.topK,
          (contactProfile == null ? void 0 : contactProfile.contact_name) || "Desconhecido",
          options == null ? void 0 : options.chatId,
          dateFrom,
          dateTo
        );
        if (contactProfile) {
