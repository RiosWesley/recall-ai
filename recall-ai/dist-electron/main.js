...
const MIGRATION_ID$b = "001_initial";
const SCHEMA_SQL$4 = `
  -- ============================================================
...
const MIGRATION_ID$a = "002_add_profile_facts";
const SCHEMA_SQL$3 = `
  -- ============================================================
...
const MIGRATION_ID$9 = "003_add_contact_profiles";
const SCHEMA_SQL$2 = `
  -- ============================================================
...
const MIGRATION_ID$8 = "004_parent_child_chunks";
const SCHEMA_SQL$1 = `
  -- ============================================================
...
function runMigration004(db) {
  const existing = db.prepare("SELECT id FROM _migrations WHERE id = ?").get(MIGRATION_ID$8);
  if (existing) {
  ...
  db.transaction(() => {
    ...
    db.prepare("INSERT INTO _migrations (id) VALUES (?)").run(MIGRATION_ID$8);
  })();
...
const MIGRATION_ID$7 = "005_propositions";
const SCHEMA_SQL = `
  -- ============================================================
...
function runMigration005(db) {
  const existing = db.prepare("SELECT id FROM _migrations WHERE id = ?").get(MIGRATION_ID$7);
  if (existing) {
  ...
  db.transaction(() => {
    ...
    db.prepare("INSERT INTO _migrations (id) VALUES (?)").run(MIGRATION_ID$7);
  })();
...
const MIGRATION_ID$6 = "006_intelligent_ingestion";
...
function runMigration006(db) {
  const existing = db.prepare("SELECT id FROM _migrations WHERE id = ?").get(MIGRATION_ID$6);
  if (existing) {
  ...
  db.transaction(() => {
    ...
    db.prepare("INSERT INTO _migrations (id) VALUES (?)").run(MIGRATION_ID$6);
  })();
  console.log("[DB] Migration 006_intelligent_ingestion complete");
}
const MIGRATION_ID$5 = "007_search_indexes";
const MIGRATION_SQL$5 = `
...
function runMigration007(db) {
  const existing = db.prepare("SELECT id FROM _migrations WHERE id = ?").get(MIGRATION_ID$5);
  if (existing) {
  ...
  db.transaction(() => {
    db.exec(MIGRATION_SQL$5);
    db.prepare("INSERT INTO _migrations (id) VALUES (?)").run(MIGRATION_ID$5);
  })();
  console.log("[DB] Migration 007_search_indexes complete");
}
const MIGRATION_ID$4 = "008_people_schema";
const MIGRATION_SQL$4 = `
  -- ============================================================
...
`;
function runMigration008(db) {
  const existing = db.prepare("SELECT id FROM _migrations WHERE id = ?").get(MIGRATION_ID$4);
  if (existing) {
  ...
  db.transaction(() => {
    db.exec(MIGRATION_SQL$4);
    db.prepare("INSERT INTO _migrations (id) VALUES (?)").run(MIGRATION_ID$4);
  })();
  console.log("[DB] Migration 008_people_schema complete");
}
const MIGRATION_ID$3 = "009_person_knowledge_schema";
const MIGRATION_SQL$3 = `
  -- ============================================================
...
const MIGRATION_ID$2 = "010_person_mentions_processed";
const MIGRATION_SQL$2 = `
...
const MIGRATION_ID$1 = "011_fix_fts_triggers";
const MIGRATION_SQL$1 = `
...
class MessageRepository {
   ...
   */
  getNeighborsByContext(sessionId, contextSnippet, limit = 4) {
    ...
    const pivot = this.db.prepare(`
      ...
      AND content LIKE ?
      LIMIT 1
    `).get(session.chat_id, session.start_time, session.end_time, `%${contextSnippet}%`);
    if (!pivot) {
      return this.db.prepare(`
        SELECT * FROM messages
        WHERE chat_id = ? AND timestamp >= ? AND timestamp <= ?
        ORDER BY timestamp ASC LIMIT ?
      `).all(session.chat_id, session.start_time, session.end_time, limit * 2);
    }
    return this.db.prepare(`
      SELECT * FROM (
        SELECT * FROM messages 
...
class PersonRepository {
   ...
   */
  createPersonWithAlias(name, alias, color) {
   ...
   */
  linkMention(sessionId, personId, context) {
  ...
  findProbableMatch(alias) {
    ...
    return this.db.prepare(query).all(matchQuery);
  }
  /**
   ...
   */
  insertTags(personId, tags, source = "map_reduce") {
    const stmt = this.db.prepare(`
   ...
   */
  insertMemory(personId, memory, sessionId = null) {
    this.db.prepare(`
...
class SessionEngine {
  ...
  group(messages) {
    ...
    let currentTokenEstimate = Math.ceil((((_a = sorted[0].content) == null ? void 0 : _a.length) || 0) / 4);
    for (let i = 1; i < sorted.length; i++) {
      const msg = sorted[i];
      const gap = msg.timestamp - lastMsgTime;
      const msgTokens = Math.ceil((((_b = msg.content) == null ? void 0 : _b.length) || 0) / 4);
...
`;
  ...
  async extractSessionEntities(sessionText) {
...
`;
    const options = {
      temperature: 0.1,
      maxTokens: 250,
      // Single session JSON ~100-200 tokens; avoids wasted generation
  ...
  async extractBatchSessionEntities(sessionsText) {
...
`;
    ...
    try {
      ...
      return results.slice(0, sessionsText.length).map((r) => ({
        summary: (r == null ? void 0 : r.summary) || "Sessão extraída",
        mentioned_entities: Array.isArray(r == null ? void 0 : r.mentioned_entities) ? r.mentioned_entities : []
      }));
    } catch (e) {
      const duration = Date.now() - startTime;
...
const _PendingMentionsManager = class _PendingMentionsManager {
  constructor() {
    __publicField(this, "db", null);
    __publicField(this, "queue", []);
  }
  static getInstance() {
    if (!_PendingMentionsManager.instance) {
      _PendingMentionsManager.instance = new _PendingMentionsManager();
    }
    return _PendingMentionsManager.instance;
  }
  /**
  ...
  addMention(sessionId, alias, context) {
    const mention = {
      id: nanoid(),
      sessionId,
      alias,
...
__publicField(_PendingMentionsManager, "instance");
let PendingMentionsManager = _PendingMentionsManager;
class ChatImportService {
  ...
  async runBackgroundNLP(chatId, rawSessions, dbSessions, sender) {
    ...
    try {
      ...
      emit({ stage: "nlp_summaries", percent: 20, label: "Extração NLP Iniciada", detail: `Processando ${totalSessions} sessões...`, chatId });
      ...
      const pendingMentionLinks = [];
      const pendingInboxMentions = [];
      for (let i = 0; i < totalSessions; i++) {
        ...
        if (textMessages.length < 3) {
          const summary2 = textMessages.length === 0 ? "Sessão sem mensagens de texto." : `Sessão breve com ${rawSess.message_count} mensagens.`;
          pendingUpdates.push({ sessionId: dbSess.id, summary: summary2, entities: [] });
          processed++;
          continue;
        }
        const sessionText = rawSess.messages.map((m) => `[${new Date(m.timestamp * 1e3).toISOString()}] ${m.sender}: ${m.content}`).join("\n");
        let result;
        try {
          result = await Promise.race([
            worker.extractSessionEntities(sessionText),
            new Promise(
              (_, reject) => setTimeout(() => reject(new Error("Session extraction timeout (15s)")), 15e3)
            )
          ]);
        } catch (e) {
      ...
      emit({ stage: "done", percent: 100, label: "Concluído", detail: `Entidades Indexadas (${totalAvg}ms/sessão).`, chatId });
    } catch (err) {
   ...
   */
  flushPendingUpdates(db, sessionRepo, personRepo, inbox, sender, updates, mentionLinks, inboxMentions) {
    db.transaction(() => {
    ...
    for (const m of inboxMentions) {
      ...
      sender == null ? void 0 : sender.send("ingest:mention_detected", pending);
    }
  }
}
...
function registerPeopleHandlers() {
  ...
  ipcMain.handle("mentions:get_people", async () => {
    ...
    const repo = new PersonRepository(db2);
    return repo.findAll();
  });
  ...
  ipcMain.handle("mentions:resolve", async (_event, mentionId, action, personId) => {
    ...
    try {
      if (action === "create_new") {
        ...
        db2.prepare(`INSERT OR IGNORE INTO person_aliases (person_id, alias) VALUES (?, ?)`).run(personId, mention.alias);
        for (const item of allToProcess) {
  ...
  ipcMain.handle("people:get_knowledge", async (_event, personId) => {
    const db2 = DatabaseService.getInstance();
    const repo = new PersonRepository(db2);
    return {
  ...
  ipcMain.handle("mentions:get_context", async (_event, sessionId, contextSnippet) => {
    const db2 = DatabaseService.getInstance();
    const repo = new MessageRepository(db2);
    return repo.getNeighborsByContext(sessionId, contextSnippet);
  });
}
const _MapReduceService = class _MapReduceService {
  constructor(intervalMs = 6e4) {
    __publicField(this, "timer", null);
    __publicField(this, "isRunning", false);
    __publicField(this, "lastRun", null);
    __publicField(this, "peopleProcessed", 0);
    __publicField(this, "totalTagsInserted", 0);
    __publicField(this, "totalMemoriesInserted", 0);
    __publicField(this, "lastError", null);
    __publicField(this, "intervalMs");
    this.intervalMs = intervalMs;
  }
  static getInstance(intervalMs = 6e4) {
    if (!_MapReduceService.instance) {
      _MapReduceService.instance = new _MapReduceService(intervalMs);
    }
    return _MapReduceService.instance;
  }
  // ─── Public API ───────────────────────────────────────────────────────────
  ...
  async runPass() {
    ...
    try {
      ...
      this.lastError = (err == null ? void 0 : err.message) ?? String(err);
      console.error("[MapReduceService] Pass failed:", this.lastError);
    } finally {
      this.isRunning = false;
    }
  }
  async processPerson(personId, mentions, repo, db) {
    const contextLines = [];
    const sessionIds = [];
    for (const m of mentions) {
...
`;
    ...
    let result;
    try {
      result = await WorkerProcess.getInstance().generateJson(prompt, options, 3);
  ...
  markProcessed(db, sessionIds, personId) {
...
__publicField(_MapReduceService, "instance", null);
let MapReduceService = _MapReduceService;
function registerMapReduceHandlers() {
