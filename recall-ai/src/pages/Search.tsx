...
function SnippetRenderer({ content, query }: { content: string, query: string }) {
  const lines = content.split('\n')
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '13px', lineHeight: '1.4' }}>
      {lines.map((line, i) => {
        // Try parsing the "Sender [Time]: Message" format generated by ChunkingEngine
        ...
        if (match) {
           ...
           return (
             <div key={i} style={{ display: 'flex', gap: '12px' }}>
                <div style={{ width: '90px', flexShrink: 0, textAlign: 'right', color: 'var(--text-muted)' }}>
                  <div style={{ fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sender.split(' ')[0]}</div>
                  <div style={{ fontSize: '9px', opacity: 0.5 }}>{time}</div>
                </div>
                <div 
                  style={{ flex: 1, color: 'var(--text-primary)', wordBreak: 'break-word', whiteSpace: 'pre-wrap' }}
          ...
          <div className="stack">
            {results.map((result, i) => (
              ...
              >
                ...
                <div className="result-card__content" style={{ marginTop: '12px', marginBottom: '16px' }}>
                  <SnippetRenderer content={result.content} query={debouncedQuery} />
                </div>
